...
subplot(2,2,3)
plot(t,x1,'r'), grid
xlabel('\lambda');ylabel('x_1(\lambda)');title('Gráfica de la señal 1 en el dominio de \lambda');
subplot(2,2,4)
plot(t,x2_1,'r'), grid 
xlabel('\lambda');ylabel('x_2(t-\lambda)');title('Gráfica de la señal 2 en el dominio de \lambda');
figure 
plot(tc,x3);grid;
xg=t1:t2;
set(gca,'xtick', xg);
axis([t1,t2,min(x3),max(x3)*1.2])
xlabel('Tiempo(t)');ylabel('x(t)*y(t)'); title('Convolución de la señal x(t) y y(t)');
